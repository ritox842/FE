...
const nonDefaultVariations = variationValues.filter(v => v !== defaultVariation);
...
function themify(options) {
   ...
   * @param variation the name of the variation. e.g. light / dark
   * @param execMode
   */
  function translateColor(colorArr, variation, execMode) {
    const colorVar = colorArr[0],
      alpha = colorArr[1];

    switch (execMode) {
      case ExecutionMode.CSS_COLOR:
        // returns the real color representation
        const color = options.pallete[variation][colorVar];
        ...
        const rgbaColorArr = getRgbaNumbers(color);
      ...
      case ExecutionMode.DYNAMIC_EXPRESSION:
        ...
        return `%[${variation}, ${colorVar}, ${alpha}]%`;
      default:
  ...
  function processRules(root) {
    root.walkRules(rule => {
      ...
      rule.walkDecls((decl, i) => {
        ...
        const colorMap = getThemifyValue(propertyValue, ExecutionMode.CSS_VAR);
        ...
        nonDefaultVariations.forEach((variationName) => {
          ...
          if (!variationRules[variationName]) {
            const clonedRule = cloneRuleWithVariation(rule, variationName);
          ...
          const variationDecl = createDecl(property, colorMap[variationName]);
          ...
          if (!ruleModeMap.hasOwnProperty(mode)) {
            ...
            variationValues.forEach(variationName => {
              ...
              if (variationName === defaultVariation) {
                ...
                newRule = cloneRuleWithVariation(rule, variationName)
              }
  ...
  function cloneRuleWithVariation(rule, variationName) {
    const selectorPrefix = `.${options.classPrefix || ''}${variationName}`;
    return postcss.rule({selector: `${selectorPrefix} ${rule.selector}`});
  }
