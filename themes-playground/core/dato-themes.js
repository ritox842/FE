...
function getThemifyValue( propertyValue ) {
  ...
  const lightVariation = value.replace(regExp, ( occurrence, value ) => {
...
module.exports = postcss.plugin('datoThemes', ( options = {} ) => {
  ...
  return css => {
    css.walkRules(rule => {
      ...
      const darkRule = createDarkRule(rule);
      ...
      rule.walkDecls(( decl, i ) => {
        ...
        if( hasThemify ) {
          const {darkVariation, lightVariation} = getThemifyValue(propertyValue);
          decl.value = lightVariation;
          darkRule.append(postcss.decl({ prop: decl.prop, value: darkVariation }));
          css.append(darkRule);
        }
      });
    });
...
function createDarkRule(rule) {
  let darkRule = rule.clone();
  darkRule.removeAll();
